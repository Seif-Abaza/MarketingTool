...
class Facebook:
    ...
    def __init__(self, settings, database, output: QListView=None, headless=False):
        ...
        if not os.path.isdir(f"{self.helper.CHROME_USER_DATA}/facebook"):
            with sync_playwright() as p:
                ...
                # Wait for login button
                    ...
                    page.fill("#email", "your_fb_username")  # Replace with self.settings["fb_username"]
                    page.fill("#pass", "your_fb_password")   # Replace with self.settings["fb_password"]
                    # Click the login button
                    page.click("//button[@name='login']")
                    # Show confirmation message
                    msg_box = QMessageBox()
                    msg_box.setWindowTitle("Confirmation")
                    msg_box.setText("This is First time only, after that you will log in normally")
    ...
    async def is_logout(self, page) -> bool:
        try:
            ...
            # Submit the form
            submit_button = login_form.locator('div[role="button"][aria-label="Accessible login button"]')
    ...
    async def get_group_members(self, category, group_name: str):
        async with async_playwright() as p:
                ...
                for group in list_of_groups:
                    ...
                    GroupName = list(filter(None, await page.get_by_role("link").all_text_contents()))[0]
        ...
        for user in users:
            ...
            async with async_playwright() as p:
                ...
                try:
                    ...
                    if len(ListOfMessages) > 0:
                        while True:
                            old_message = self.database.search_by_columns("total_msg_post", 'user', user["name"], 'message', send_message)
                            if not old_message:
                                break
                            else:
                                send_message = ListOfMessages[
                    ...
                    self.helper.UpDateOutput(f"Message for {user['name']} is {send_message}")
                    # Wait for the "Message" button and click it
                    message_button = await page.wait_for_selector('//div[@aria-label="Message"]', timeout=10000)
                    await message_button.click()
                    # await page.screenshot(path=f"{user['name']}_0.png")
                    # Wait for message input box
                    message_box = await page.wait_for_selector('//div[@aria-label="Message" and @role="textbox"]', timeout=10000)
                    ...
                    self.database.save_msg_or_post(user=user["name"], message=send_message, source="FBM")
