...
import random
import string
from PySide6.QtWidgets import QMessageBox, QListView , QProgressDialog , QDialog
...
class helper:
    ...
    async def hide_browser(self, user_data_dir, browser_path, headless=True):
        return await launch(
                headless=headless,
                ...
                args=[
                    '--no-sandbox',
                    f'--user-data-dir={user_data_dir}/default',
                    '--disable-blink-features=AutomationControlled'
                ]
        )
    
    def _browser_(self, driver_name:str, user_dir:str, driver_path:str, withheadless: bool=False):
        chrome_options = Options()
        chrome_user_data_dir = user_dir + f'/{driver_name}'
        ...
        if os.path.exists(chrome_user_data_dir) and os.path.isdir(chrome_user_data_dir):
            if withheadless:
        ...
        service = Service(chrome_driver_path, service_args=['--log-level=INFO', '--disable-build-check'], log_output=f'{user_dir}/{driver_name}_log.txt')
        driver = webdriver.Chrome(service=service, options=chrome_options)
        if withheadless == False:
            ...
            return joined_time.strftime("%Y-%m-%d %H:%M:%S")
                ...
                for word in words:
                    if word in days_map:  # Check if the word is a weekday
                        ...
                        if days_difference == 0:  # If today is the target day, return today's date
    ...
    def load_settings(self, app_path:str):
        ...
        settings = {
            ...
            'session': app_path_abs + "/" + config_parser.get(section_name, 'SESSION_FILE'),
            'log': app_path_abs + "/" + config_parser.get(section_name, 'LOG_FILE'),
            'output': app_path_abs + "/" + config_parser.get(section_name, 'DEFAULT_OUTPUT'),
            'wait': config_parser.get(section_name, 'WAIT_TIME'),
            'app_path': app_path_abs,
            'message': self.read_file_as_text(config_parser.get(section_name, 'MESSAGE_PATH')),
...
class AI_Window(QDialog, AI_Dialog):
    ...
    def __init__(self, parent=None, command:list=None, CompanName:str=None, Compan_ID:str=None, Source:str=None, Database=None):
    ...
    def process_data(self, DataList:list):
        try:
            if isinstance(DataList, list):
                processed_list = [msg.split(":", 1)[1].strip() for msg in DataList if ":" in msg]
    ...
    def ok_clicked(self):
        ...
        for msg in self.ResultFinal:
            self.database.save_msg_history(self.CompanName, self.Compan_ID, msg, self.Source)
        self.accept()
