...
class SMSServices():
    ...
    def send_top_country_for_service(self,ServiceName:str) -> list:
        ListOfService = []
        if ServiceName:
            TopServiec = self.SMS.getTopCountriesByService(self.get_service_code(ServiceName))
            for item in TopServiec:
                NumberOfServif = TopServiec[item]['count'] - 1
                if NumberOfServif > 0:
                    ListOfService.append({
                        'country_code':TopServiec[item]['country'],
                        'country_name': self.CountrysInfo[str(TopServiec[item]['country'])]['eng'],
    ...
    def get_service_order(self,phone_number):
        ...
        DataFromDB = self.database.read_from_database(self.TABLE_NAME,{'code':'','phone':phone_number})
        if DataFromDB:
            record = DataFromDB[0]
            ...
            if activations['status'] == 'success':
                ...
                self.database.update_in_database(self.TABLE_NAME,{'id':record['id']},DataToDB)
